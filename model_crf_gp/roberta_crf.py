...
config = CrfBertConfig.from_pretrained('/opt/lijianlong/gaiic/chinese-roberta-wwm',
                                  num_labels=len(ner_train_dataset.cat2id))
dl_module = CrfBert.from_pretrained('/opt/lijianlong/gaiic/chinese-roberta-wwm',
                                    config=config)
...
model = Task(dl_module, optimizer, 'ce', scheduler=schedule, cuda_device=2)
...
torch.save(model.module.state_dict(), 'robert_crf_people.pth')
